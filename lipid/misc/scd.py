...
def s_cd(ndx_f, trj_f):
	...
	# 109.5/2 degrees in radians.
	phi = 0.95557
	z = np.array([0, 0, 1])

	for snap_t in xrange(0,n_snaps):
		tail_length = len(trj_qs[snap_ts[snap_t]])
		s_cd = {}

		for c in xrange(len(n_ls)):
			scd1s = []
			scd2s = []
			...
			for l in range(n_qs):
				...
				cd_bisector = -1 * rotation(v_post, nrml_v, -1 * angl/2)
